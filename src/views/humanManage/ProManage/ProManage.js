 ...
 * @LastEditTime: 2021-11-24 13:21:21
...
let params = "";
let frompage1 = "";
import {perListByManager,persDelete} from '@/api/api.js'
...
export default {
  ...
      let that = this;
      ...
      perListByManager(params).then(res => {
          if(res.code == 200){
            that.loading = false;
...
        })
    ...
    let name = tag?'当前页':'全部';
		aLink.setAttribute('download',`管理人员导出${name}数据` +'.csv')// 设置下载文件名称
		aLink.click();
