 ...
 * @LastEditTime: 2021-12-22 16:01:10
...
import {attendList,attendListMj,exportCurrentList,exportCurrentAllList} from '@/api/api.js'
...
      let params = {
        ...
        limit:that.pageSize,
        persType:that.activeName=='first'?2:3,
...
     	let params = {
      ...
      let name = that.activeName=='first'?'管理人员':'备案人员'
      this.loading = true;
      if(tag){
  ...
  //         aLink.setAttribute('download',name+'导出当前页数据' +'.csv')// 设置下载文件名称
  //         aLink.click()
  ...
  //         let blob =new Blob([res], {type:"application/vnd.ms-excel"})
  //         aLink.href = URL.createObjectURL(blob)
  //         aLink.setAttribute('download',name+'导出全部数据' +'.csv')// 设置下载文件名称
  //         aLink.click()
  //         this.$refs.loadElement.appendChild(aLink)
  //         	})
  //         	.catch(() => {})
